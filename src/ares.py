...
from src.param_scanner import generate_param_scan_variants
...
def _evaluate_run_worker(args: tuple) -> tuple:
    ...
    (Fixed for VS Code: Injects PYTHONPATH but respects existing files)
    """
    ...
    try:
        # 3. Prepare the evaluation environment
        original_problem_dir = os.path.join(root_dir, "problems", problem_name)

        # Check if source exists
        if not os.path.exists(original_problem_dir):
            raise FileNotFoundError(f"Problem directory not found at: {original_problem_dir}")

        eval_env_dir = os.path.join(temp_dir, "problem")

        # [CRITICAL] Copy all existing files (including the original main.py if it exists)
        shutil.copytree(original_problem_dir, eval_env_dir)

        # 4. Write the individual's evolved code to gpt.py
        # We do NOT touch main.py, assuming it exists and serves a specific purpose.
        ...
        # --- [VS CODE FIX] Ensure PYTHONPATH is correct for subprocess ---
        # In VS Code, PYTHONPATH is often empty. We must manually add the project root.
        # This allows the script to import utils or other modules located in root.
        current_env = os.environ.copy()
        python_path = current_env.get("PYTHONPATH", "")
        # Prepend the project root directory to PYTHONPATH
        current_env["PYTHONPATH"] = f"{root_dir}{os.pathsep}{python_path}"
        # --- [END FIX] ---
        ...
        with open(final_stdout_path, 'w', encoding='utf-8') as f_stdout:
            subprocess.run(
                ...
                timeout=timeout,
                check=True,
                cwd=eval_env_dir,  # Run inside the temp directory
                env=current_env  # Pass the fixed environment variables
            )
        ...
        # If any error occurs (timeout, crash, etc.), log it to the stdout file
        error_message = f"\n--- EVALUATION FAILED IN SUBPROCESS ---\n{type(e).__name__}: {e}\n"
        try:
            if final_stdout_path:
                with open(final_stdout_path, 'a', encoding='utf-8') as f_stdout:
                    f_stdout.write(error_message)
            else:
                print(f"CRITICAL: No stdout path available to log error: {error_message}")
...
class ARES:
    ...
    def _parse_innovation_proposals(self, proposal_text: str) -> List[dict]:
        """
        Parses the Theorist's proposal to extract a LIST of atomic <experiment> blocks.

        Each experiment must be exactly one of:
          - Structural: requires <factor_name_to_ablate> (non-empty), and no parameter scan.
          - Parametric: requires <parameter_name_to_scan> (non-empty), and no scan direction/multiplier.
        """
        logging.info("Parsing Theorist's multi-experiment proposal...")
        ...
        if not experiment_blocks:
            logging.warning("No <experiment> tags found. Treating entire text as a single proposal.")
            experiment_blocks = [proposal_text]
        ...
        for i, block in enumerate(experiment_blocks):
            ...
            # 1) Type parsing aligned with schema: "[Type] Structural" or "[Type] Parametric"
            type_match = re.search(r"\[Type\]\s*(Structural|Parametric)\b", raw, re.IGNORECASE)
            if type_match:
                t = type_match.group(1).strip().capitalize()
                info["type"] = f"{t} Innovation"

            # 2) Extract targets
            ablation_match = re.search(
                r"<factor_name_to_ablate>(.*?)</factor_name_to_ablate>",
                raw, re.DOTALL | re.IGNORECASE
            )
            if ablation_match:
                info["factor_to_ablate"] = (ablation_match.group(1) or "").strip()

            param_match = re.search(
                r"<parameter_name_to_scan>(.*?)</parameter_name_to_scan>",
                raw, re.DOTALL | re.IGNORECASE
            )
            if param_match:
                info["parameter_to_scan"] = (param_match.group(1) or "").strip()

            # 3) Validation: enforce atomic experiments & required fields
            t = info["type"]
            fac = (info["factor_to_ablate"] or "").strip()
            par = (info["parameter_to_scan"] or "").strip()

            if t == "Unknown":
                logging.warning(f"  - Skipped Proposal #{i + 1}: Could not determine [Type].")
                continue

            if fac and par:
                logging.warning(f"  - Skipped Proposal #{i + 1}: Both factor and parameter specified (not atomic).")
                continue

            if t == "Structural Innovation":
                if not fac:
                    logging.warning(f"  - Skipped Proposal #{i + 1}: Structural without factor_name_to_ablate.")
                    continue
                info["parameter_to_scan"] = ""

            elif t == "Parametric Innovation":
                if not par:
                    logging.warning(f"  - Skipped Proposal #{i + 1}: Parametric without parameter_name_to_scan.")
                    continue
                info["factor_to_ablate"] = ""

            else:
                logging.warning(f"  - Skipped Proposal #{i + 1}: Unsupported innovation type '{t}'.")
                continue

            proposals.append(info)
            logging.info(
                f"  - Extracted Proposal #{i + 1}: "
                f"Type='{info['type']}', "
                f"Ablation='{info['factor_to_ablate']}', "
                f"Scan='{info['parameter_to_scan']}'"
            )

        logging.info(f"Total valid experiments parsed: {len(proposals)}")
        return proposals
    ...
    def init_prompt(self) -> None:
        ...
        # Optional init-only description for Theorist initialization
        init_txt_path = f'{problem_prompt_path}/init.txt'
        if os.path.exists(init_txt_path):
            self.init_description = file_to_string(init_txt_path).strip()
            logging.info(f"Loaded init-only description from: {init_txt_path}")
        else:
            self.init_description = ""
            logging.warning(f"Optional init-only description file not found: {init_txt_path}")
        ...
        self.user_generator_context = f"""
            ...
            {self.seed_func}
            """
        # Slim context: only for Experimenter
        self.experimenter_context = f"""
        [Function Signature]
        {self.func_signature}
        """
    ...
    def init_population(self) -> None:
        ...
        prompt = self.theorist_init_prompt.format(
            description=self.init_description,
        ...
        prompt = self.experimenter_prompt.format(
            user_generator_context=self.experimenter_context,
    ...
    def build_individual_from_code(
            self,
            code: str,
        ...
        """
        Build an individual directly from a code string, without passing through
    ...
    def run_critic_validation(self, best_ind: dict, hypothetical_table: str, proposal: str) -> dict:
        """
        Critic Validation [Multi-Request, 1 experiment = 1 request]:
        1) Parse proposal into <experiment> blocks.
        2) For each experiment:
           - Structural -> critic_ablation_prompt -> LLM generates mutant code(s)
           - Parametric -> local parameter scanner generates mutant code(s)
        3) Evaluate all mutants
        4) Aggregate report
        """
        logging.info("Critic is designing validation experiments (Multi-Request Mode, 1 exp = 1 req)...")
        ...
        innovation_list = self._parse_innovation_proposals(proposal)
        if not innovation_list:
            return {
                "report_markdown": "# Critic's Report\n\nSkipped: Failed to parse any valid <experiment> blocks.",
                "experiment_details": [],
                "evaluated_mutants": []
            }

        max_requests = int(self.cfg.get("critic_validation_max_requests", len(innovation_list)))
        all_mutant_population = []

        for i, exp_info in enumerate(innovation_list):
            if i >= max_requests:
                break

            logging.info(
                f"--- Processing Critic Request {i + 1}/{min(len(innovation_list), max_requests)}: {exp_info['type']} ---"
            )

            innovation_type = exp_info["type"]
            factor_to_ablate = (exp_info.get("factor_to_ablate") or "").strip()
            parameter_to_scan = (exp_info.get("parameter_to_scan") or "").strip()

            if innovation_type == "Structural Innovation":
                prompt = self.critic_ablation_prompt.format(
                    best_code=best_ind["code"],
                    strategic_innovation_proposal=exp_info["raw_text"],
                ...
                try:
                    ...
                    logging.error(f"Critic LLM call failed for experiment #{i + 1}: {e}")
                    continue

                output_path = (
                    f"{self.critic_output_dir}/mutant_generation_raw_iter_{self.iteration}_exp_{i + 1}_{tag}.txt"
                )
                with open(output_path, "w", encoding="utf-8") as f:
                    f.write(response)

                batch_mutant_codes = re.findall(r"```python(.*?)```", response, re.DOTALL | re.IGNORECASE)
                if not batch_mutant_codes:
                    logging.warning(f"Critic produced no python code blocks for exp #{i + 1}.")
                    continue

                for k, code in enumerate(batch_mutant_codes):
                    unique_id = f"critic_iter{self.iteration}_exp{i + 1}_mut{k}"
                    ind = self.response_to_individual(code, unique_id, role="critic")

                    try:
                        first_line = code.strip().splitlines()[0]
                        if first_line.startswith("#"):
                            ind["description"] = (
                                    f"[Exp {i + 1} {innovation_type}] " + first_line.lstrip("#").strip()
                            )
                        else:
                            ind["description"] = f"[Exp {i + 1} {innovation_type}] Mutation"
                    except Exception:
                        ind["description"] = f"[Exp {i + 1}] Malformed code."

                    all_mutant_population.append(ind)

            elif innovation_type == "Parametric Innovation":
                tag = "parameter_scan"

                try:
                    scan_result = generate_param_scan_variants(
                        best_ind["code"],
                        parameter_to_scan,
                        max_variants=int(self.cfg.get("critic_param_scan_max_variants", 4))
                    )
                    variants = scan_result["variants"]
                except Exception as e:
                    logging.error(
                        f"Local parameter scan failed for experiment #{i + 1} on '{parameter_to_scan}': {e}"
                    )
                    continue
                ...
                with open(archive_path, "w", encoding="utf-8") as f:
                    f.write(scan_result["scan_log"])
                    f.write("\nGenerated variants:\n")
                    for variant in variants:
                        f.write(f"- {variant['mutation_note']}\n")

                if not variants:
                    logging.warning(
                        f"Local parameter scan produced no valid variants for exp #{i + 1} on '{parameter_to_scan}'."
                    )
                    continue

                for k, variant in enumerate(variants):
                    unique_id = f"critic_iter{self.iteration}_exp{i + 1}_param{k}"
                    description = f"[Exp {i + 1} Parametric Innovation] {variant['mutation_note']}"
                    ind = self.build_individual_from_code(
                        variant["code"],
                        unique_id,
                        role="critic",
                        description=description,
                ...
                logging.warning(f"Unknown type {innovation_type} in experiment #{i + 1}. Skipping.")
                continue

        if not all_mutant_population:
            logging.warning("Critic produced no valid mutants across all requests.")
            return {
                "report_markdown": "# Critic's Report\n\nNo valid mutants were produced.",
                "experiment_details": [],
                "evaluated_mutants": []
            }

        logging.info(
            f"Critic generated a total of {len(all_mutant_population)} mutants across "
            f"{min(len(innovation_list), max_requests)} requests."
        )
        evaluated_mutants = self.evaluate_population(all_mutant_population)
        ...
        report_md += (
            f"**Experiment Overview**: Parsed {len(innovation_list)} experiment blocks; "
            f"executed {min(len(innovation_list), max_requests)} validation requests.\n\n"
        )
        report_md += "| Test ID | Type/Description | Performance Score | Delta vs. Best | Conclusion |\n"
        report_md += "|---|---|---|---|---|\n"
        ...
        for detail in experiment_details:
            if detail["id"] == "Baseline":
                report_md += (
                    f"| Baseline | Original Best Code | {detail['score']:.4f} | +0.0000 | Reference |\n"
                )
                continue

            clean_description = str(detail["description"]).replace("Mutation - ", "").replace(":", "").strip()
            ...
            if isinstance(detail["score"], float):
                ...
                if conclusion == "Improvement":
                    conclusion = "**Improvement**"
                report_md += (
                    f"| {detail['id']} | {clean_description} | {detail['score']:.4f} | "
                    f"{delta:+.4f} | {conclusion} |\n"
                )
            else:
        ...
        logging.info(f"Combined Critic's report generated and saved to {report_path}")
    ...
    def run_theorist_final_reflection(self, phase1_outputs: tuple, critic_results: dict,
        ...
        if self.stagnation_counter >= self.radical_exploration_trigger:
            ...
            prompt = self.theorist_radical_prompt.format(
                ...
                # anti_pattern_lib ����prompt���Ѵ�table���ȡ
            )
            temperature += 0.4
        ...
        try:
            while self.function_evals < self.cfg.max_fe:
                ...
                if successful_critic_inds:
                    ...
                    # --- Logic Split: Local vs Global ---

                    # 1. Local Comparison: Did Critic beat the current generation's best?
                    # We update this so the Theorist (Step 4) analyzes the freshest, best code available.
                    if best_critic_ind["obj"] < best_ind_this_iteration["obj"]:
                        local_improvement = best_ind_this_iteration["obj"] - best_critic_ind["obj"]
                        logging.info(f"*** CRITIC LOCAL IMPROVEMENT ***")
                        logging.info(
                            f"  - Critic code '{best_critic_ind['response_id']}' beat the local population best.")
                        logging.info(f"  - Score: {best_critic_ind['obj']:.4f} (Local Delta: {local_improvement:.4f})")

                        # Update the local pointer for the current workflow
                        best_ind_this_iteration = best_critic_ind

                        # 2. Global Comparison: Did this ALSO break the historical record?
                        # ONLY update the official ARES state if it beats the all-time best.
                        if best_critic_ind["obj"] < self.best_obj_overall:
                            global_improvement = self.best_obj_overall - best_critic_ind["obj"]
                            logging.info(f"*** NEW GLOBAL BEST RECORD! ***")
                            logging.info(f"  - The Critic's code also beat the historical best!")
                            logging.info(
                                f"  - Old Best: {self.best_obj_overall:.4f} -> New Best: {best_critic_ind['obj']:.4f}")

                            # Update Global State
                            self.best_obj_overall = best_critic_ind["obj"]
                            self.best_code_overall = best_critic_ind["code"]
                            self.best_code_path_overall = best_critic_ind["code_path"]
                            self.elitist = best_critic_ind
                            self.stagnation_counter = 0  # Reset stagnation only on global breakthrough
                        else:
                            logging.info(
                                f"  - (Note: Did not beat global best of {self.best_obj_overall:.4f}, global state unchanged)")
                ...
                try:
                    table_match = re.search(
                        r"^##\s*3\.\s*\[Final Strategy Table\]\s*(.*?)(?=^##\s*\d+\.|\Z)",
                        final_guide,
                    ...
                    if table_match:
                        ...
                        has_core_cols = (
                                "Factor/Strategy" in new_strategy_table
                                and "Role/Hypothesis" in new_strategy_table
                                and "Known Pitfalls" in new_strategy_table
                        )
                        looks_like_md_table = "|" in new_strategy_table and "---" in new_strategy_table

                        logging.info(f"RAW EXTRACTED TABLE START:\n{repr(new_strategy_table[:1000])}")
                        logging.info(f"has_core_cols={has_core_cols}, looks_like_md_table={looks_like_md_table}")

                        if has_core_cols and looks_like_md_table:
                            self.strategy_table_str = new_strategy_table
                            logging.info("Central strategy table has been successfully updated.")
                        else:
                            logging.warning(
                                "Extracted content from [Final Strategy Table] does not appear to be a valid reduced markdown table. State will not be updated."
                            )
                    else:
                        logging.warning(
                            "Could not find the exact section header '## 3. [Final Strategy Table]' in the final guide. The state will not be updated for this iteration."
                        )
                except Exception as e:
